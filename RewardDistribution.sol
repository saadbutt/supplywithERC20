...
contract RewardDistribution is Ownable {
    ...
    constructor(address _signerAddress,address _tokenAddress) {
        require(_signerAddress != address(0), "Signer Address could not be empty");
        require(_tokenAddress != address(0), "Token Address could not be empty");
    ...
    function withdrawReward(address userAddress, uint256 amount,uint256 withdrawalId,bytes memory signature) public  returns(address) {
        require(userAddress != address(0), "User Address could not be empty");
        require(amount > 0, "Amount cannot be zero");
        require(usedSignatures[signature] == false,"Signatures already used");
        address recoveredAddress;
        recoveredAddress =  recoverSigner(keccak256(abi.encodePacked(userAddress, amount, withdrawalId)) ,signature);
